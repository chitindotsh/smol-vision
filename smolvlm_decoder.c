/*
 * smolvlm_decoder.c - SmolLM2 decoder (Llama-style)
 *
 * Architecture (per layer):
 *   RMSNorm -> QKV (no bias) -> NeoX RoPE
 *   -> Causal MHA attention -> Output proj -> residual
 *   RMSNorm -> SwiGLU MLP (gate/up/down, no bias) -> residual
 *
 * Key differences from Qwen3 decoder:
 *   - NO per-head Q/K RMSNorm
 *   - MHA: n_kv_heads = n_heads = 32 (not GQA)
 *   - head_dim = 64 (not 128)
 *   - rope_theta = 273768.0 (not 1e6)
 *   - Separate lm_head (not tied to embeddings)
 *   - rms_norm_eps = 1e-5 (not 1e-6)
 */
...
static int kv_cache_init(smolvlm_ctx_t *ctx, int max_seq) {
...
static int kv_cache_grow(smolvlm_ctx_t *ctx, int required) {
...
static float *kv_k_at(smolvlm_ctx_t *ctx, int layer, int pos) {
    int kv_dim = ctx->config.dec_kv_heads * ctx->config.dec_head_dim;
    return ctx->kv_cache_k + ((size_t)layer * ctx->kv_cache_max + pos) * kv_dim;
}

static float *kv_v_at(smolvlm_ctx_t *ctx, int layer, int pos) {
    int kv_dim = ctx->config.dec_kv_heads * ctx->config.dec_head_dim;
    return ctx->kv_cache_v + ((size_t)layer * ctx->kv_cache_max + pos) * kv_dim;
}

static int ensure_prefill_buffers(smolvlm_ctx_t *ctx, int seq_len) {
...
static int ensure_rope_inv_freq(smolvlm_ctx_t *ctx, int head_dim, float theta) {
    int half = head_dim / 2;
...
static int ensure_rope_cache(smolvlm_ctx_t *ctx, int required_pos, int head_dim, float theta) {
...
void smolvlm_decoder_prefill(smolvlm_ctx_t *ctx, const float *input_embeds, int seq_len) {
    smolvlm_decoder_t *dec = &ctx->decoder;
    const smolvlm_config_t *cfg = &ctx->config;
    int dim = cfg->dec_hidden;
    ...
    for (int layer = 0; layer < cfg->dec_layers; layer++) {
        smolvlm_dec_layer_t *l = &dec->layers[layer];
...
static void ensure_dec_buffers(smolvlm_ctx_t *ctx) {
    if (ctx->dec_x) return;
    const smolvlm_config_t *cfg = &ctx->config;
    int dim = cfg->dec_hidden;
    int q_dim = cfg->dec_heads * cfg->dec_head_dim;
...
int smolvlm_decoder_forward(smolvlm_ctx_t *ctx, const float *input_embed) {
    ...
    if (pos >= ctx->kv_cache_max) {
        if (kv_cache_grow(ctx, pos + 1024) != 0) return SMOLVLM_TOKEN_EOS;
    }

    if (ensure_rope_cache(ctx, pos + 1, head_dim, theta) != 0) {
        return SMOLVLM_TOKEN_EOS;
    }
    ...
    /* Final norm + argmax against lm_head (NOT tok_embeddings) */
