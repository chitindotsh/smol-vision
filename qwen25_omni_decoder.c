/*
 * qwen25_omni_decoder.c - Qwen2.5-Omni-7B LLM decoder
 *
 * Architecture differences from Qwen3:
 *   - QKV have biases (O does not)
 *   - No per-head Q/K RMSNorm
 *   - Separate lm_head (not tied to embeddings)
 *   - NeoX RoPE (mRoPE degenerates for text/audio)
...
static float *load_f32(multi_safetensors_t *ms, const char *name) {
    ...
    if (!t) {
        fprintf(stderr, "q25 decoder: weight not found: %s\n", name);
        return NULL;
    }
...
int q25_decoder_load(q25_decoder_t *dec, void *ms_void, const q25_config_t *cfg) {
    ...
    for (int i = 0; i < cfg->dec_layers; i++) {
        ...
        snprintf(name, sizeof(name), "thinker.model.layers.%d.self_attn.k_proj.bias", i);
        ...
        snprintf(name, sizeof(name), "thinker.model.layers.%d.self_attn.v_proj.bias", i);
        l->wv_bias = load_f32(ms, name);

        /* O weight only (no bias) */
        ...
        if (!l->wq_weight_bf16 || !l->wk_weight_bf16 ||
            ...
            fprintf(stderr, "q25 decoder: failed to load attn weights for layer %d\n", i);
            return -1;
        }
        if (!l->wq_bias || !l->wk_bias || !l->wv_bias) {
            fprintf(stderr, "q25 decoder: failed to load attn biases for layer %d\n", i);
            return -1;
        }
        ...
        if (!l->gate_weight_bf16 || !l->up_weight_bf16 || !l->down_weight_bf16) {
            fprintf(stderr, "q25 decoder: failed to load MLP weights for layer %d\n", i);
            return -1;
        }

        /* Fuse gate+up: interleave rows [gate_row0, up_row0, ...] */
        {
...
static int kv_cache_init(q25_ctx_t *ctx, int max_seq) {
...
static int kv_cache_grow(q25_ctx_t *ctx, int required) {
...
static float *kv_k_at(q25_ctx_t *ctx, int layer, int pos) {
    int kv_dim = ctx->config.dec_kv_heads * ctx->config.dec_head_dim;
    return ctx->kv_cache_k + ((size_t)layer * ctx->kv_cache_max + pos) * kv_dim;
}

static float *kv_v_at(q25_ctx_t *ctx, int layer, int pos) {
    int kv_dim = ctx->config.dec_kv_heads * ctx->config.dec_head_dim;
    return ctx->kv_cache_v + ((size_t)layer * ctx->kv_cache_max + pos) * kv_dim;
}

static int ensure_prefill_buffers(q25_ctx_t *ctx, int seq_len) {
...
static int ensure_rope_inv_freq(q25_ctx_t *ctx, int head_dim, float theta) {
    int half = head_dim / 2;
...
static int ensure_rope_cache(q25_ctx_t *ctx, int required_pos, int head_dim, float theta) {
...
void q25_decoder_prefill(q25_ctx_t *ctx, const float *input_embeds, int seq_len) {
    ...
    for (int layer = 0; layer < cfg->dec_layers; layer++) {
        q25_dec_layer_t *l = &dec->layers[layer];

        /* Input RMSNorm */
        qwen_rms_norm(x_norm, x, l->input_norm, seq_len, dim, eps);

        /* QKV with biases */
        qwen_linear_bf16(q, x_norm, l->wq_weight_bf16, l->wq_bias,
                          seq_len, dim, q_dim);
        qwen_linear_bf16(k, x_norm, l->wk_weight_bf16, l->wk_bias,
                          seq_len, dim, kv_dim);
        qwen_linear_bf16(v, x_norm, l->wv_weight_bf16, l->wv_bias,
                          seq_len, dim, kv_dim);
...
static void ensure_dec_buffers(q25_ctx_t *ctx) {
    if (ctx->dec_x) return;
    const q25_config_t *cfg = &ctx->config;
    int dim = cfg->dec_hidden;
    int q_dim = cfg->dec_heads * cfg->dec_head_dim;
...
int q25_decoder_forward(q25_ctx_t *ctx, const float *input_embed) {
    ...
    if (pos >= ctx->kv_cache_max) {
        if (kv_cache_grow(ctx, pos + 1024) != 0) return Q25_IM_END;
    }
    ...
    for (int layer = 0; layer < cfg->dec_layers; layer++) {
        ...
        /* Fused QKV matvec (no bias), then add biases */
        qwen_linear_nobias_bf16_qkv(q, k, v, x_norm,
                                    ...
                                    dim, q_dim, kv_dim);
        /* Add QKV biases */
        qwen_add_inplace(q, l->wq_bias, q_dim);
        qwen_add_inplace(k, l->wk_bias, kv_dim);
        qwen_add_inplace(v, l->wv_bias, kv_dim);
        ...
        memcpy(kv_k_at(ctx, layer, pos), k, kv_dim * sizeof(float));
        memcpy(kv_v_at(ctx, layer, pos), v, kv_dim * sizeof(float));
    ...
    /* Final norm + streaming argmax via SEPARATE lm_head */
    qwen_rms_norm(x, x, dec->norm, 1, dim, eps);
    return qwen_argmax_matvec_bf16(x, dec->lm_head_bf16, dim, cfg->vocab_size);
}

/* ========================================================================
 * Decoder Forward — Full Logits Variant
 * ======================================================================== */
...
void q25_decoder_forward_logits(q25_ctx_t *ctx, const float *input_embed, float *logits) {
    ...
    /* Final norm + full lm_head matmul → logits (SEPARATE lm_head) */
    qwen_rms_norm(x, x, dec->norm, 1, dim, eps);
    qwen_matmul_t_bf16(logits, x, dec->lm_head_bf16, 1, dim, cfg->vocab_size);
}
